...
**Centers and roles.** Each village center has its own inventory (`center_inventory`), orders and operator; `available = on_hand - reserved`. Set `SUPER_ADMIN_EMAILS` (comma-separated) to name the platform administrators: they manage everything under `/v1/admin`: operators and farmers (categorised, suspend/reactivate), centers, orders, restock approvals, and an audit log. Operators only reach `/v1/operator` once they own a center. Nothing about centers is seeded. App orders hold stock at their center (`reserved`), are assigned automatically to the best center that has the whole cart, are moved to another center if theirs closes, is suspended or goes offline, and expire after 5 days if not collected (a background job; see `src/services/reservationJobs.js`). `POST /v1/centers/nearby` ranks centers for a farmer by distance (Haversine, adaptive 10/20/35 km radius) and stock together; the scoring rules are in `src/services/centerRanking.js` and documented in `API.md`.
